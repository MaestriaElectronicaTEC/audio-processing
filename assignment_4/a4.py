...
from numpy import amax, argmax, arange, concatenate, interp, sum, zeros
from numpy.random import randn
from scipy.signal import hann, sawtooth

def a4(x, fm, plim=None, dt=None, n=None, tw=None, dlog2p=None, s0=None): # subharmonic-to-harmonic ratio
    #VARIABLES
    ...
    for i in range(0, len(pc)): # para cada candidato:
        k = arange(1,n+1)
        k_05 = k - 0.5
        fi = pc[i]*k # identifique primeras n armonicas
        fi_05 = pc[i]*k_05
        A = zeros((len(fi), X.shape[1])) # cree matriz nArmonicas x nVentanas
        for j in range(0, X.shape[1]): # por cada ventana:
            A[:,j] = (interp(fi, f, X[:,j]) / k) - (interp(fi_05, f, X[:,j]) / k_05) # estime el espectro en las armonicas
