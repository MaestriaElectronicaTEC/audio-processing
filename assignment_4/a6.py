...
def a6(x, fm, plim=None, dt=None, tw=None, dlog2p=None, s0=None): # Autocorrelation with proportion correction
    #VARIABLES
    ...
    for i in range(0, len(pc)): # para cada candidato:
        ...
        for j in range(0, X.shape[1]): # por cada ventana:
            spectrum = interp(fi, f, X[:,j])
            spectrum = spectrum**0.5
            A[:,j] = scalar * spectrum * cosine
